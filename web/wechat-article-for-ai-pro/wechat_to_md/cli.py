...
from .formatter import format_standard_output
from .ocr_processor import generate_ocr_output
...
async def process_single_article(
    ...
    try:
        ...
        # 9. Format to standard 7-file structure (with original HTML and article_id)
        article_id = format_standard_output(
            article_dir=article_dir,
            url=url,
            meta=meta,
            original_md_name=f"{safe_title}.md",
            html_raw=html,
        )

        # 10. Generate OCR output with image slicing
        from .ocr_processor import process_all_images, create_ocr_markdown_v2, read_article_markdown
        image_ocr_results, qr_results = process_all_images(article_dir)
        article_text = read_article_markdown(article_dir / "article.md")
        create_ocr_markdown_v2(article_dir, article_text, image_ocr_results, qr_results)

        # 11. Copy to ~/.hermes/output/{article_id}/ for Hermes integration
        hermes_output_dir = Path.home() / ".hermes" / "output" / article_id
        if hermes_output_dir.exists():
            shutil.rmtree(hermes_output_dir)
        shutil.copytree(article_dir, hermes_output_dir)
        logger.info(f"Copied to Hermes output: {hermes_output_dir}")

        logger.info(f"Saved: {article_dir} (ID: {article_id}, {len(final_md)} chars, {len(parsed.image_urls)} images)")
        return True
